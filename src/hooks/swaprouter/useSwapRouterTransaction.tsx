...
export default function useSwapRouterTransaction(
  ...
  return useMemo(() => {
    ...
    return {
      callback: async function onSwapRouter(): Promise<TransactionResponse> {
        ...
        return provider
          ...
          .catch((error) => {
            ...
            if (error?.code === 4001) {
              ...
              console.error(`SwapRouter Transaction failed`);

              throw new Error(`SwapRouter Transaction failed: ${error.reason ?? error.message ?? ''}`);
            }
          });
      },
    };
